...
public sealed class UpdateFareUseCase : IUpdateFareUseCase
{
    private readonly IFareRepository _repository;

    public UpdateFareUseCase(IFareRepository repository)
    {
    ...
    {
        var x = Fare.Create(FareId.Create(request.Id), FareRouteId.Create(request.RouteId), FareCabinTypeId.Create(request.CabinTypeId), FarePassengerTypeId.Create(request.PassengerTypeId), FareSeasonId.Create(request.SeasonId), FareBasePrice.Create(request.BasePrice), FareValidFrom.Create(request.ValidFrom), FareValidTo.Create(request.ValidTo));
