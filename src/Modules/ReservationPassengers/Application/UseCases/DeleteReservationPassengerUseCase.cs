...
public interface IDeleteReservationPassengerUseCase
{
    Task ExecuteAsync(int id, CancellationToken cancellationToken = default);
}

public sealed class DeleteReservationPassengerUseCase : IDeleteReservationPassengerUseCase
{
    private readonly IReservationPassengerRepository _repository;

    public DeleteReservationPassengerUseCase(IReservationPassengerRepository repository)
    {
    ...
    public Task ExecuteAsync(int id, CancellationToken cancellationToken = default)
    {
        ...
        return _repository.DeleteAsync(ReservationPassengerId.Create(id), cancellationToken);
    }
}
