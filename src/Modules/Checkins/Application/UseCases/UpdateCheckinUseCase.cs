...
public sealed class UpdateCheckinUseCase : IUpdateCheckinUseCase
{
    ...
    )
    {
        ...
        var x = Checkin.Create(CheckinId.Create(request.Id), CheckinTicketId.Create(request.TicketId), CheckinStaffId.Create(request.StaffId), CheckinFlightSeatId.Create(request.FlightSeatId), CheckinDate.Create(request.CheckinDate), CheckinStatusId.Create(request.CheckinStatusId), CheckinBoardingPassNumber.Create(boardingPassNumber), CheckinHasCheckedBaggage.Create(request.HasCheckedBaggage), CheckinBaggageWeightKg.Create(request.BaggageWeightKg));
